...
int s21_floor(s21_decimal value, s21_decimal *result) {
  ...
  // Переменная для хранения числа 0 в форме s21_decimal
  ...
  if (code == 0) {
    ...
    if (value_sign == 1 && s21_is_greater(fractional, zero)) {
      // Если число отрицательное и дробная часть больше 0, прибавляем 1
      code = s21_add(unsigned_tr_value, one, result);
    } else {
      // Иначе просто записываем в результат число без дробной части
